...
@section('content')
    <section class="mb-5" aria-labelledby="categories-kpi-heading">
        <h2 id="categories-kpi-heading" class="sr-only">Categories overview</h2>
    ...
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <form action="{{ route('admin.categories.index') }}" method="get" class="flex flex-wrap gap-2 w-full sm:max-w-md">
            <input
                type="text"
                name="search"
                value="{{ $search ?? '' }}"
                        ...
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3 min-w-0">
                                    <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary-light text-lg">
                                        {{ $category->icon ?: '📁' }}
                                    </span>
                                    <span class="font-medium text-navy truncate">{{ $category->name }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <span class="inline-flex px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 text-xs font-mono">{{ $category->slug }}</span>
                            </td>
