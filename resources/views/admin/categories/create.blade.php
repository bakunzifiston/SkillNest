    ...
    <div class="max-w-xl">
        <form action="{{ route('admin.categories.store') }}" method="post" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="icon" id="icon" value="{{ old('icon') }}" placeholder="e.g. 💻 or dev" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-accent focus:ring-accent">
                @error('icon')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div class="flex gap-3">
                <button type="submit" class="px-4 py-2 bg-accent text-white rounded-lg font-medium hover:bg-accent-dark">Create category</button>
