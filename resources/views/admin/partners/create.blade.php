    ...
    <div class="max-w-xl">
        <form action="{{ route('admin.partners.store') }}" method="post" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Acme Inc" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-accent focus:ring-accent">
            ...
            <div class="p-4 bg-accent-light/50 rounded-xl border border-accent-muted">
                ...
                <input type="file" name="logo" id="logo" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp,image/svg+xml" required class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-accent file:text-white file:font-medium hover:file:bg-accent-dark">
            ...
            <div class="flex gap-3">
                <button type="submit" class="px-4 py-2 bg-accent text-white rounded-lg font-medium hover:bg-accent-dark">Add partner</button>
