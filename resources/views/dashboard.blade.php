                ...
                @if($bundleEnrollments->isNotEmpty())
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <ul class="space-y-3">
                                @foreach($bundleEnrollments as $be)
                                    ...
                                    <li class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                                        <a href="{{ route('bundles.show', $be->bundle) }}" class="font-medium text-gray-900 hover:text-primary">{{ $be->bundle->title }}</a>
                                        <div class="flex items-center gap-3">
                                            <span class="text-sm text-gray-600">{{ $be->completed_courses }} / {{ $be->total_courses }} courses</span>
                                            <span class="text-sm font-semibold text-primary">{{ (int) $be->bundle_completion_percentage }}%</span>
                            ...
                            <a href="{{ route('bundles.my-bundles') }}" class="inline-block mt-4 text-sm text-primary font-medium hover:underline">View all my bundles →</a>
                        </div>
                    </div>
                @endif
            @endunless
