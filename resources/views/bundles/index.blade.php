        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($bundles->isEmpty())
                <div class="text-center py-16 rounded-2xl bg-slate-50 border border-slate-100">
                    ...
                    <a href="{{ route('courses.index') }}" class="mt-4 inline-block text-primary font-semibold hover:underline">Browse courses</a>
                </div>
            @else
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    @foreach($bundles as $bundle)
                    <a href="{{ route('bundles.show', $bundle) }}" class="group block bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl hover:border-accent-muted transition">
                        ...
                        <div class="p-5">
                            <h2 class="font-display font-semibold text-slate-900 group-hover:text-accent-dark line-clamp-2">{{ $bundle->title }}</h2>
                            <p class="mt-2 text-sm text-slate-500 line-clamp-2">{{ Str::limit($bundle->description, 80) }}</p>
                            <p class="mt-3 text-sm font-medium text-primary">{{ $bundle->courses_count }} courses</p>
                        </div>
                    </a>
                    @endforeach
                </div>
