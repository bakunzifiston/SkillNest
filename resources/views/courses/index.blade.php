    ...
    <section class="py-12 lg:py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap gap-2 mb-10">
                <a href="{{ route('courses.index') }}" class="px-4 py-2 rounded-full text-sm font-medium {{ !request('category') ? 'bg-accent text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">All</a>
                @foreach($categories as $cat)
                <a href="{{ route('courses.index', ['category' => $cat->slug]) }}" class="px-4 py-2 rounded-full text-sm font-medium {{ request('category') === $cat->slug ? 'bg-accent text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">{{ $cat->name }}</a>
                @endforeach
            </div>
            @endif
            ...
            @if($courses->isEmpty())
                <div class="text-center py-16 rounded-2xl bg-slate-50 border border-slate-100">
                    ...
                    <a href="{{ route('home') }}" class="mt-4 inline-block text-primary font-semibold hover:underline">Back to Home</a>
                </div>
            @else
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    @foreach($courses as $course)
                    <article class="group block bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl hover:border-accent-muted transition">
                        <a href="{{ route('courses.show', $course) }}" class="block">
                            ...
                            <div class="p-5">
                                ...
                                <span class="text-xs font-medium text-primary uppercase tracking-wide">{{ $course->category->name }}</span>
                                @endif
                                <h2 class="mt-2 font-display font-semibold text-slate-900 group-hover:text-accent-dark line-clamp-2">{{ $course->title }}</h2>
